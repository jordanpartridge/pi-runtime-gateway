...
export class Runtime extends EventEmitter {
  constructor({ root, profile, stateDir, spawnProcess = spawn }) {
    super(); Object.assign(this, { root, profile, stateDir, spawnProcess });
    this.runs = new Map();
  }
  start({ prompt, project }) {
    ...
    const run = { id, project, status: 'starting', startedAt: new Date().toISOString(),
      ...
      events: [], hooks: [], text: '', directory, auditPath, auditLines: 0, eventBytes: 0,
    ...
    // Do not inherit provider credentials or global agent configuration.
  ...
  handle(run, e) {
    if (e.type === 'response' && e.id === 'boot') {
      ...
      if (e.message.stopReason === 'error') this.fail(run, 'provider_error');
      ...
      if (!run.targetStatus) run.targetStatus = run.text ? 'completed' : 'failed';
      if (!run.text && !run.error) run.error = 'empty_result';
      this.stop(run);
    }
  }
  collectAudit(run) {
  ...
  snapshot(run) {
    const keys = ['id','project','status','startedAt','finishedAt','profile','provider','model','pid','sessionId',
      'initialMessageCount','text','error','stopReason','childExited','exitCode','exitSignal','cancelRequestedAt','abortAcknowledged'];
