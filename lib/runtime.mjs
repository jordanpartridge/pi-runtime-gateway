...
export class Runtime extends EventEmitter {
  ...
  start({ prompt, project, chat }) {
    ...
    const run = { id, project, status: 'starting', startedAt: new Date().toISOString(),
      ...
      stderrBytes: 0, stderrHash: createHash('sha256') };
  ...
  collectAudit(run) {
    let raw;
    try {
      raw = readFileSync(run.auditPath, 'utf8');
    } catch { return; }
    if (raw.length > 1000000) return this.fail(run, 'audit_limit');
    const lines = raw.split('\n'); lines.pop();
    while (run.auditLines < lines.length) {
      const lineNumber = ++run.auditLines;
      try {
        const value = JSON.parse(lines[lineNumber - 1]); run.hooks.push(value); this.event(run, 'hook', value);
      } catch {
        run.auditErrorCount = (run.auditErrorCount || 0) + 1;
        if (run.auditErrorLine === undefined) {
          run.auditErrorLine = lineNumber;
          this.event(run, 'audit_error', { error: 'invalid_audit_json', line: lineNumber });
        }
        if (!run.targetStatus) this.fail(run, 'invalid_audit_json');
      }
    }
  }
  event(run, type, data) {
  ...
  finalize(run) {
    ...
    run.stderrSha256 = run.stderrHash.digest('hex'); delete run.stderrHash;
    if (run.auditErrorCount) {
      run.status = 'failed'; run.error = 'invalid_audit_json';
    } else if (run.status === 'completed' && !run.hooks.some(h => h.hook === 'runtime.before_provider_request')) {
  ...
  snapshot(run) {
    const keys = ['id','project','status','startedAt','finishedAt','profile','provider','model','pid','sessionId',
      ...
      'stderrBytes','stderrSha256','auditErrorCount','auditErrorLine'];
