...
const PROVIDER_SLUG = /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/;
const KEY_REFERENCE = '$PI_GATEWAY_PROVIDER_API_KEY';
...
export function loadProfile({ root, env = process.env } = {}) {
  ...
  const envText = [['PI_GATEWAY_PI_BINARY', 'piBinary'], ['PI_GATEWAY_PROVIDER', 'provider'], ['PI_GATEWAY_MODEL', 'model'], ['PI_GATEWAY_OLLAMA_URL', 'ollamaUrl']];
  ...
  if (typeof profile.provider !== 'string' || profile.provider.length > 80 || !PROVIDER_SLUG.test(profile.provider))
    throw configError('provider must be a lowercase Pi provider slug.');
  text(profile.model, 'model', 300);
  const workerEnv = {};
  if (profile.provider !== 'ollama') {
    if (profile.model === DEFAULTS.model)
      throw configError('Select a model supported by the configured cloud provider using PI_GATEWAY_MODEL or the profile model setting.');
    if (!own(env, 'PI_GATEWAY_API_KEY') || typeof env.PI_GATEWAY_API_KEY !== 'string' || !env.PI_GATEWAY_API_KEY.trim())
      throw configError('PI_GATEWAY_API_KEY is required for a cloud provider.');
    workerEnv.PI_GATEWAY_PROVIDER_API_KEY = text(env.PI_GATEWAY_API_KEY, 'PI_GATEWAY_API_KEY', 16384);
  }
  ...
  delete profile.port;
  return { profile, stateDir, port, workerEnv };
}
...
function isManagedAuth(auth) {
  return plainObject(auth) && Object.entries(auth).every(([provider, credential]) =>
    provider !== 'ollama' && provider.length <= 80 && PROVIDER_SLUG.test(provider)
    && plainObject(credential) && Object.keys(credential).length === 2
    && credential.type === 'api_key' && credential.key === KEY_REFERENCE);
}

/** Generate isolated Pi configuration with environment references, never actual cloud keys. */
export function prepareAgent(profile, stateDir) {
  ...
  try {
    ...
    let authStat, existingAuth = {};
    try { authStat = lstatSync(authPath); }
    catch (error) { if (error.code !== 'ENOENT') throw error; }
    if (authStat) {
      if (!authStat.isFile() || authStat.isSymbolicLink() || authStat.size > 4096)
        throw configError('The isolated agent auth file must be a regular JSON file containing only managed references.');
      try { existingAuth = JSON.parse(readFileSync(authPath, 'utf8')); }
      catch { throw configError('The isolated agent auth file must be a regular JSON file containing only managed references.'); }
      if (!isManagedAuth(existingAuth))
        throw configError('The isolated agent auth file must not contain credentials or unmanaged references.');
    }
    ...
    if (profile.provider === 'ollama') {
      ...
      if (Object.keys(existingAuth).length) writePrivateJson(authPath, {});
    } else {
      // Pi supplies the built-in provider catalog. The worker receives the key
      // through its environment; reusable files contain only this fixed reference.
      writePrivateJson(resolve(agentDir, 'models.json'), { providers: {} });
      writePrivateJson(authPath, { [profile.provider]: { type: 'api_key', key: KEY_REFERENCE } });
    }
    writePrivateJson(resolve(agentDir, 'settings.json'), { defaultProvider: profile.provider, defaultModel: profile.model,
